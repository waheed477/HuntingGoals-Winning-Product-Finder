...
export default function AlertsSection() {
  ...
  const fetchHistory  = useStore((s) => s.fetchAlertHistory)
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <div className="glass-card p-6 space-y-5">
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="range" min={0} max={100} value={form.minScore}
        ...
        <div className="pt-4 border-t border-white/10">
          <h4 className="text-sm font-medium text-gray-300 mb-3">Daily Digest</h4>
      ...
      <div className="space-y-4">
        <div className="glass-card p-5">
          <h3 className="text-sm font-semibold text-white mb-3">Active Alerts ({alerts.length})</h3>
          {alerts.length === 0 ? (
              ...
              {alerts.map((alert) => (
                <div key={alert.id} className="flex items-center gap-3 p-3 bg-white/3 rounded-xl border border-white/5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white font-medium truncate">{alert.city} · {alert.category}</p>
                    <p className="text-xs text-gray-500">Min score: {alert.minScore} · via {alert.notifyVia}</p>
                  </div>
                  <button
